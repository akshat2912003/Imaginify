...
export const Collection = ({
  hasSearch = false, images, totalPages = 1, page,
}: {
  images: IImage[]; totalPages?: number; page: number; hasSearch?: boolean;
}) => {
    ...
    <>
      ...
      {images && images.length > 0 ? (
        <ul className="collection-list">
          {images.map((image) => <Card image={image} key={image._id as string} />)}
        </ul>
      ) : (
...
const Card = ({ image }: { image: IImage }) => {
  ...
  const typeInfo = transformationTypes[typeKey];
    ...
    <li>
      <Link href={`/transformations/${image._id}`} className="collection-card">
        ...
        <div className="flex-between">
          ...
          <span className="text-xl" title={typeInfo?.title}>{typeInfo?.icon || "🖼️"}</span>
        </div>
      </Link>
    </li>
  );
